...
app.get('/api/dia/resumen', (req, res) => {
  ...
  const porEmp = all(`
    SELECT e.id, e.nombre,
      COALESCE((SELECT SUM(bm.cantidad) FROM bar_movimientos bm WHERE bm.empleada_id=e.id AND bm.fecha=? AND bm.tipo IN ('venta','salio','manual')),0) -
      ...
      COALESCE((SELECT SUM(bm.cantidad*bm.precio_unit) FROM bar_movimientos bm WHERE bm.empleada_id=e.id AND bm.fecha=? AND bm.tipo IN ('venta','salio','manual')),0) -
