...
const { getDB, run, all, get, insert, save, saveNow } = require('./database');
...
app.get('/api/cervezas', (_,res) => {
  ...
  const h = hoy();
  rows.forEach(c => {
    c.precio = parseFloat(c.precio)||0;  // garantizar número
...
app.post('/api/mesas/cantidad', (req,res) => {
  ...
  if (cantidad > actual) {
    for (let i=actual+1;i<=cantidad;i++) run('INSERT OR IGNORE INTO mesas (id,estado) VALUES (?,?)',[i,'libre']);
  }
...
app.post('/api/pedidos', (req,res) => {
  ...
  // Forzar precios numéricos para evitar concatenación de strings
...
app.post('/api/bar/movimientos', (req,res) => {
  ...
  const h=hoy(), hr=hora();
  const id = insert('INSERT INTO bar_movimientos (empleada_id,mesa_id,cerveza_id,marca,cantidad,tipo,precio_unit,fecha,hora) VALUES (?,?,?,?,?,?,?,?,?)',
    [empleada_id,mesa_id||null,cerveza_id||null,marca,cantidad,tipo,precio_unit||0,h,hr]);
  if (tipo!=='devol') run('UPDATE cervezas SET stock=MAX(0,stock-?) WHERE id=?',[cantidad,cerveza_id]);
  else run('UPDATE cervezas SET stock=stock+? WHERE id=?',[cantidad,cerveza_id]);
  res.json({id});
});

app.get('/api/bar/turno/:empleada_id', (req,res) => {
  const h=hoy(), eid=req.params.empleada_id;
  const out = get('SELECT COALESCE(SUM(cantidad*precio_unit),0) as t, COALESCE(SUM(cantidad),0) as q FROM bar_movimientos WHERE empleada_id=? AND tipo!=? AND fecha=?',[eid,'devol',h]);
  const dev = get('SELECT COALESCE(SUM(cantidad*precio_unit),0) as t, COALESCE(SUM(cantidad),0) as q FROM bar_movimientos WHERE empleada_id=? AND tipo=? AND fecha=?',[eid,'devol',h]);
  const mesa = get('SELECT COALESCE(SUM(total),0) as t FROM pedidos WHERE empleada_id=? AND fecha=? AND cobrado=0',[eid,h]);
  const movs = all('SELECT hora,marca,cantidad,tipo FROM bar_movimientos WHERE empleada_id=? AND fecha=? ORDER BY id DESC LIMIT 10',[eid,h]);
  const porMarca = all('SELECT marca, SUM(CASE WHEN tipo!=? THEN cantidad ELSE -cantidad END) as qty, MAX(precio_unit) as precio FROM bar_movimientos WHERE empleada_id=? AND fecha=? GROUP BY marca',['devol',eid,h]).filter(r=>r.qty>0);
  res.json({
    total: ((out?.t||0)-(dev?.t||0))+(mesa?.t||0),
    cervezas: (out?.q||0)-(dev?.q||0),
    movimientos: movs,
    porMarca,
    mesaTotal: mesa?.t||0
  });
...
app.post('/api/bar/cobrar-turno', (req,res) => {
  ...
  const h=hoy();
  run('UPDATE pedidos SET cobrado=1 WHERE empleada_id=? AND fecha=? AND cobrado=0',[empleada_id,h]);
...
app.post('/api/caja/cerrar', (req,res) => {
  ...
  saveNow(); // guardar caja inmediatamente
...
getDB().then(() => {
  app.listen(PORT, '0.0.0.0', () => {
    ...
    Object.values(nets).flat().filter(n=>n.family==='IPv4'&&!n.internal).forEach(n=>{
      console.log(`   📱 Celular/Red: http://${n.address}:${PORT}`);
    });
    console.log('🌸 ==========================================\n');
  });
}).catch(err => {
...
app.get('/api/dia/resumen', (req, res) => {
  ...
  // Por empleada: cervezas y ventas del día
  const porEmp = all(`
  ...
  // Movimientos bar del día por empleada (últimos 10 por empleada)
  ...
  // Mesas activas (ocupadas hoy con pedidos sin cobrar)
...
// ── DETALLE DE MESA (pedidos actuales) ──────────────────
...
app.delete('/api/dia/reset', (req, res) => {
  const h = hoy();
  try {
    // Borrar items de pedidos de hoy
    ...
    // Restaurar stock de cervezas (sumar lo que se vendió hoy de vuelta)
